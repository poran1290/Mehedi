...
const text = {
  bn: {
    ...
    welcomeText: "আপনার স্মার্ট ইনভেস্টমেন্ট যাত্রা শুরু করুন।",
    secureAccount: "অ্যাকাউন্ট সুরক্ষিত রাখুন",
    secureText: "সবসময় শক্তিশালী পাসওয়ার্ড ব্যবহার করুন।",
    subtitle: "স্মার্ট ইনভেস্টমেন্ট প্ল্যাটফর্ম",
    accountAccess: "অ্যাকাউন্ট অ্যাক্সেস",
    accountText: "লগইন করুন অথবা নতুন অ্যাকাউন্ট তৈরি করুন",
    login: "লগইন",
    ...
    terms: "আমি শর্তাবলিতে সম্মত",
    createAccount: "অ্যাকাউন্ট তৈরি করুন",
    alreadyAccount: "ইতিমধ্যে অ্যাকাউন্ট আছে?",
    loginNow: "লগইন করুন",
    rights: "সর্বস্বত্ব সংরক্ষিত।",
    recoverTitle: "পাসওয়ার্ড পুনরুদ্ধার",
    recoverText: "আপনার ফোন নম্বর দিন। পুনরুদ্ধারের নির্দেশনা পাঠানো হবে।",
    ...
    shortPassword: "কমপক্ষে ৮ অক্ষরের শক্তিশালী পাসওয়ার্ড দিন।",
  ...
  en: {
    ...
    welcome: "Welcome to RCN",
    welcomeText: "Start your smart investment journey.",
    secureAccount: "Keep your account secure",
    secureText: "Always use a strong password.",
    subtitle: "Smart Investment Platform",
    accountAccess: "Account Access",
    accountText: "Login or create a new account",
    login: "Login",
    register: "Register",
    phone: "Phone Number",
    password: "Password",
    ...
    shortPassword: "Use a strong password with at least 8 characters.",
...
function updatePasswordStrength() {
  ...
  passwordHint.textContent =
    registerPassword.value ? labels[score] : "";
}

function setLoading(button, status) {
  button.classList.toggle("loading", status);
}
...
function validateLogin() {
  ...
  if (!validPhone(phone.value.trim())) {
    showError(phone, translate("invalidPhone"));
    valid = false;
  } else {
    clearError(phone);
  }
  ...
  if (!password.value.trim()) {
    showError(password, translate("required"));
    valid = false;
  } else {
    clearError(password);
  }
...
function validateRegister() {
  ...
  if (name.value.trim().length < 2) {
    showError(name, translate("required"));
    valid = false;
  } else {
    clearError(name);
  }
  ...
  if (passwordScore(password.value) < 3) {
    showError(password, translate("shortPassword"));
    valid = false;
  } else {
    clearError(password);
  }
  ...
  if (!confirmPassword.value ||
    ...
    showError(confirmPassword, translate("passwordMismatch"));
    valid = false;
  } else {
  ...
  if (!terms.checked) {
    showMessage(translate("acceptTerms"));
    valid = false;
  }
...
loginForm.addEventListener("submit", (event) => {
  ...
  simulateSubmit(button, translate("loginSuccess"));
});
...
registerForm.addEventListener("submit", (event) => {
  ...
  simulateSubmit(button, translate("registerSuccess"), () => {
    registerForm.reset();
    updatePasswordStrength();
  });
});
...
recoveryForm.addEventListener("submit", (event) => {
  ...
  const button = recoveryForm.querySelector(".submit-button");

  if (!validPhone(phone.value.trim())) {
    phone.classList.add("invalid");
    error.textContent = translate("invalidPhone");
    return;
  }
  ...
  setTimeout(() => {
    setLoading(button, false);
    closeModal();
    showToast(translate("recoverySuccess"));
  }, 900);
});
